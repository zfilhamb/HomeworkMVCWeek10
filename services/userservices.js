const UserRepository = require("../repositories/userRepository.js");


class UserServices {

    static loginUser = async (email, password, next) => {
        try {
            const data = await UserRepository.loginUser(email, password, next);
            return data;
        } catch (err) {
            next(err)
        }
    }

    static registerUser = async (id, email, gender, password, role, next) => {
        try {
            const data = await UserRepository.registerUser(
                id, email, gender, password, role, next);
            return data;
        } catch (err) {
            next(err)
        }
    }

    static changePassword = async (id, password, next) => {
        try {
            const data = await UserRepository.changePassword(id, password, next);
            return data;
        } catch (err) {
            next(err)
        }
    }

    static deleteUser = async (id, next) => {
            try {
                const data = await UserRepository.deleteUser(id, next);
                return data;
            } catch (err) {
                next(err)
            }
        }
}
module.exports = UserServices;